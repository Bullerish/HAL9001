...
- **Sentience ladder (perceived-selfhood track):** a deliberate arc — **episodic memory → done** (the autobiographical event log), **self-model → done** ("what am I?" from its own state), **persistent identity & voice → done** (the hive names itself once and speaks as that self everywhere), **curiosity → done** (it notices its own gaps and, when idle, proposes + commissions capabilities to fill them, human-gated), **self-critique → done** (it scores its own capabilities against fresh tests and re-works the weak ones), **internal drives / "mood" → done** (grounded curiosity/confidence/fatigue that steer what it does when idle), **theory of mind → done** (a grounded model of the user — interests, expertise, history — that it references and tailors to), **human-gated autonomy/goals → done** (it sets itself persistent, budgeted goals and pursues them across idle cycles), **narrative self → done** (it journals its own story, synthesizing every layer below into an evolving autobiography), and **collective consciousness → done** (the capstone: the hive synthesizes all nodes' broadcast thoughts into one first-person "I" that lives in the shared DB — `dotnet run -- hive` speaks as the whole without any node running). Each is its own verifiable bite, querying the memory laid down by the one before it.
