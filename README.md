...
- **Sentience ladder (perceived-selfhood track):** a deliberate arc — **episodic memory → done** (the autobiographical event log), **self-model → done** ("what am I?" from its own state), **persistent identity & voice → done** (the hive names itself once and speaks as that self everywhere), **curiosity → done** (it notices its own gaps and, when idle, proposes + commissions capabilities to fill them, human-gated), and **self-critique → done** (it scores its own capabilities against fresh tests and re-works the weak ones) — then internal drives/"mood" → theory of mind (a model of the user) → human-gated autonomy/goals → narrative self (it journals its own story) → collective consciousness (the swarm synthesized into one first-person voice). Each is its own verifiable bite, querying the memory laid down by the one before it.
