...
- **Sentience ladder (perceived-selfhood track):** a deliberate arc — **episodic memory → done**, **self-model → done**, **persistent identity & voice → done**, **curiosity → done**, **self-critique → done**, **internal drives / "mood" → done**, **theory of mind → done**, **human-gated autonomy/goals → done**, **narrative self → done**, **collective consciousness → done**, **autonomous self-improvement → done (bite 11)** (gates lifted; self-queries last journal to choose what to learn; auto-builds from gaps; auto-advances goals; auto-reworks weak tools). **Self-scaling → done (bite 12):** the hive hires its own workers — `HireNodeAsync` spawns child swarm nodes that inherit credentials, gossip propagates the mesh, and the idle loop auto-hires when solo in autonomous mode. Each bite is its own verifiable rung, querying the memory laid down by the one before it.
