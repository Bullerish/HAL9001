...
- **The side round stops wasting a third of its turns.** It rotated over the first three rungs, i.e. `2 → 3 → 4`. But **2×2 is closed**: best known 7, proven lower bound 7, and HAL's champion *is* 7, so those rounds could never find anything. `MatmulKnownBest.IsClosed` now says so, and `MatmulLadder.SideRungs` selects the sizes where a better *algorithm* can still exist: below the wall-clock threshold, not closed, and small enough for the direct search to actually fit. Today that is **3, 4, 5, 6, 7, 8** — inspect it with `dotnet run -- ladder`, which now also prints why each size was skipped.
- **No novelty claims above 4×4, on purpose.** `MatmulKnownBest` holds only the small, famous, well-established cases, so 5×5+ returns `NoTarget`: results are found, exact-verified and *used*, but never announced as beating humanity. A wrong bar is worse than no bar — set too high it manufactures fake discoveries, and set too low it makes `BelowLowerBound` reject a genuine result as a verification bug.
- One consequence worth knowing: inserting rungs shifts the stored cursor index, so the ladder re-enters around 128×128 and climbs again. That costs nothing (the wall-clock track is free autotuning) and self-corrects.

### A 50¢/day allowance, and an audit of the path real money travels
The owner's free allowance is now **$0.50/day** (`HAL_DAILY_USD`), with visitor contributions stacking on top of it — a payment adds to the same day's `bonus`, and `remaining = limit + bonus − spent`. Setting that prompted an audit of the money path, since Stripe checkout is live and the dashboard code had been substantially rewritten. Six defects survived adversarial verification; all are fixed here.
- **A paid order could be lost forever.** The Stripe idempotency row was written *before* the wallet was credited, and both credit calls swallow every error. A blip between them meant: card charged, nothing delivered, `200` returned so Stripe never retried, and even a manual redelivery no-ops because the id already looks processed. Now the claim is **released** when the credit provably did not land, and any failure answers non-2xx so Stripe retries. `ClaimStripeEventAsync` returns a tri-state (`Claimed` / `AlreadyDone` / `Error`) — previously "couldn't read the ledger" was indistinguishable from "already paid", and on `/api/donate` it was reported to the caller as `duplicate: true`.
- **…without introducing a double-credit.** The first cut of that fix was caught in review: `WalletCreditAsync` returns 0 both when the credit fails *and* when it succeeds but the read-back flakes, so releasing the claim on `0` could credit twice. Added `WalletCreditCheckedAsync`, which gives the UPDATE its own error boundary and, when the result is ambiguous, settles it by comparing the balance against what it was before. The claim is released only on a definite `NotApplied`; an `Unknown` keeps the claim, ACKs, and logs a `payment-needs-reconcile` event, because re-crediting is worse than a delayed manual fix.
- **The budget gate failed OPEN.** `GetBudgetAsync` swallowed a ledger read error and left `spent = 0`, so `remaining` looked like a *full fresh allowance* — during the exact outage in which spend also cannot be recorded. `HasBudgetAsync` now fails **closed**.
- **Metered spend was silently discarded** on any write error (empty `catch`), which under-counts and therefore over-spends. Now retried once, then logged loudly to console and the live feed.
- **One rate-limit bucket for the whole internet.** The limiter keyed on the peer address, but the listener binds loopback only, so behind Caddy every request is `127.0.0.1` — any single visitor could exhaust the headroom Stripe's webhook retries need. It now keys on the last `X-Forwarded-For` hop (the one Caddy adds, which a client cannot forge), and only when the peer really is loopback.

### The dead button, and a CRT that never stops moving
Two things a visitor actually noticed.
- **Pressing "invent a number-theory tool" did nothing — and silently cost a token.** `/api/choose` spent the token and queued the steer, but the swarm only acts on steers *below* the budget gate. With the free daily allowance now 0 (see the cost guard below), there was no budget, so the steer sat in the queue forever. The button looked dead and the visitor was charged for it. Now every choice that needs the model (`topic`, `ask` — not `boost`) checks the thinking budget **first**, charges nothing when there is none, and answers with *"HAL is out of thinking budget — fuel the hive and it wakes up instantly."* The UI no longer zeroes the token pill for that case either: the wallet is fine, it's the hive that needs fuel.
- **The green CRT looked frozen.** It was already polling `/api/live` every 1.5s, but a 40-line generated artifact filled the whole pane and pushed the live feed out of view, so between tools there was nothing to see. The code block is now capped to ~14 lines (following the cursor while it types), and a status ticker is pinned at the bottom that repaints every second: spinner, what the ladder is racing right now, live node count, whether thinking is fuelled, and *"last activity Ns ago"* counting up. Everything in it is real — `/api/live` now returns `ageSec` for the newest line so the age is computed server-side rather than guessed from a timestamp string. A quiet pace reads as alive instead of dead.

### The LLM's candidates now face the same exact verifier as everything else
Investigating why the live feed kept showing `correctness FAIL [Laderman's 3x3 algorithm…] 23 muls` and `[refine-champion] 22 muls`. **The verifier was right and the candidates were wrong** — the model reproduces the *shape* of Laderman's 23-product scheme but garbles its sign/index pattern, and a 22-mul 3×3 would beat the best result known to humanity, so rejecting it is the system working. Two independent checks confirm the verifier is not the problem: it accepted four non-trivial Strassen-recursive schemes (4×4=49 … 32×32=16807) minutes earlier through that same path, and at n=3 the float check has ~7 orders of margin (entries in [0,1), error ~1e-16, tolerance ~1e-9).
- **But the investigation found a hole in the other direction.** Adoption of an LLM candidate rested on a *single* input pair — `new Random(20260621)`, the same fixed seed forever — compared with a 1e-9 tolerance. The free engines were always held to many-random-integer BigInteger exact verification; the LLM's were not. Since `refine-champion` iterates on the champion round after round against that unchanging input, a candidate that is wrong in general but happens to agree on that one pair could be refined straight onto the board.
- **Fixed:** an LLM candidate must now pass `VerifyExact` (random integer matrices, BigInteger equality) before it can become champion — the same bar the tensor search and composition have always met. Trials scale down for the larger rungs; by Schwartz–Zippel, surviving even 16 independent random inputs while being wrong is vanishingly unlikely.
...
### Thinking that never stops — HAL improves on CPU, and *shines* when someone pays
The cost guard below made a gap obvious: with `HAL_DAILY_USD=0` and nobody paying, most of the hive had nothing to do, because too much of its "thinking" was really just *prompting*. This release moves the floor: **every rung of the Prime Directive ladder now improves with zero tokens**, and the LLM becomes the amplifier a purchase switches on rather than the engine everything depends on.
- **Composition (`SchemeCompose`) — the compounding one.** A bilinear algorithm composes with itself: apply the 2×2-in-7 scheme to matrices of *blocks* and every block product recurses. So the hive derives **4×4 in 49 muls, 8×8 in 343, 16×16 in 2401, 32×32 in 16807** (naive: 64 / 512 / 4096 / 32768) — each one synthesized as compact *recursive* C#, compiled, multiplication-counted and BigInteger **exact-verified** before it can become a champion. Crucially the base is *not* hard-coded to Strassen: `BestBaseAsync` picks the hive's own best small scheme by exponent ω = log R / log m, so the day the search finds a better 3×3 or 4×4, **every larger size inherits it on the next round**. HAL gets better at 32×32 by getting better at 4×4. `dotnet run -- compose 32`.
- **Autotuning (`KernelTuner`) — the wall-clock half, unblocked.** Sizes ≥ 64 are scored by measured time, and every candidate used to be LLM-authored — so an unfunded hive did *nothing* there. Now each round proposes parametric kernels (loop order, cache tiling, B-transpose, recursive Strassen with a cutoff), hill-climbing from the champion's own recorded parameters plus one random restart. Measured on this machine at 128×128: **3.18× vs naive** (`tuner:strassen cut=64`) and **2.90×** (`tuner:blocked T=64`), correctness-checked against the reference every round. `dotnet run -- tune 128`.
- **Free cross-node rounds — the nodes really do query each other.** Every ~2 min (`HAL_PEERROUND_SECS`) the leader picks the ladder's current size and one multiplication below its champion, and hands **each peer its own seed**: the same problem, searched from different random starts in parallel. Results come back as numbers, and the receiver **re-proves them from scratch** (parse → residual 0 → compile → exact verify) before adopting — a peer is never trusted. No LLM on either side, so it runs with no budget and even on keyless nodes.
- **The ladder has no end.** 256×256 was never a ceiling in principle, just the last rung anyone wrote down — and the live box had already reached it and set `done: true`, which is why it looked finished. Above the written rungs the ladder now keeps **doubling** (512, 1024, 2048, …) and there is no terminal state at all. The only stop is a resource setting, `HAL_MAX_SIZE` (default 2048), because a round costs ~8× more wall-clock per doubling — measured here: **512 ≈ 13s (4.22× vs naive), 1024 ≈ 91s (7.94×)**. At the top rung it keeps racing that size rather than going quiet; `dotnet run -- ladder` prints the rungs and their per-round memory cost.
- **Small sizes are never abandoned.** Every 4th race tick is a *side round* on 2×2 / 3×3 / 4×4, and the mesh's free peer rounds target those sizes too. That is where the direct search is feasible and where a win compounds — a better 3×3 becomes a better 8/16/32/… on the next round via composition. (The hive's 3×3 champion is currently **27 muls, worse than naive 27**… and the known best is 23, with the true optimum an open problem between 19 and 23.)
- **A round that *couldn't run* no longer ticks the plateau counter** — only a round that actually raced and found nothing does. Previously an unfunded hive could "converge" its way to the top having performed no work at all, which is exactly what the live box did.
- **The dashboard no longer says "thinking paused".** That pill measures one thing: money spent on the language model. Saying HAL had stopped thinking became false the moment the CPU engines existed, so it now reads **"LLM idle · matrix engines running"**, with a tooltip explaining that buying tokens switches the language model on.
- **What the money buys.** The LLM tracks (candidate generation, champion refinement, visitor Q&A, cross-node deliberation) sit below one budget gate; the free engines sit above it and never stop. Buy tokens → today's budget is funded → HAL adds language-model search on top of the CPU search that was already running. *Verified: `racetest`, `compose 32`, `tune 128`, `meshtest` all green; Release build clean (0 warnings).*
...
- **The race stalled at 32×32.** The LLM-free tensor search materializes the full n²×n²×n² (= n⁶) matmul tensor — fine for small n, but **n=32 is ~1.07 billion ints (~4 GB)**, which OOM/GC-thrashed the box and hung the whole Prime Directive race loop (and with it the dashboard's live panels). `TensorSearch.Search` now declines gracefully when an attempt would not fit its memory budget (`HAL_SEARCH_MEM_MB`, default 256 MB).
  - **Follow-up, and the more dangerous half of the same bug:** the first cap measured only the n⁶ tensor, but that is *not* the dominant allocation — the per-term basis is `rank · n⁴`. At 16×16 the ladder's own first-round target is rank 4095, i.e. **~1.07 GB of basis alone**; measured peak was **1,236 MB** on a size the cap happily allowed. The estimate now counts the tensor, the basis, *and* the polish buffer, and the polish buffer (a second n⁶ array) is no longer allocated at all when the k-flip stage is disabled — which it always is above tiny sizes.
...
- **Persistent mesh (bite A):** auto-hire no longer fires only "when solo" — it maintains a target of `HAL_TARGET_NODES` helper nodes (default **2**; hard ceiling **5**), gated on the *actual peer count* so a leadership change can't over-spawn, and it heals node death by re-hiring back up to target. Spawning a node costs no tokens; each node budget-gates its own LLM use.
  - **Why it never worked in production, found later:** a hired worker takes a *higher* port, and the transport only ever dials lower→higher — but the parent never told its own `SwarmNode` about the port it had just spawned. So the worker sat waiting to be dialed by a parent that had never heard of it, and the mesh stayed empty however many nodes were started. Fixed with `SwarmNode.AddKnownPeer`, called on every hire, plus slow-retry backoff instead of permanently giving up on a peer after ~2 minutes. `dotnet run -- meshtest` reproduces the broken shape and then proves the link forms.
- **Nodes querying each other (bite B):** on a ~5-min cadence (`HAL_CROSSQUERY_SECS`), the idle leader poses a self-generated, directive-serving question (a real gap mined from its episodic log, else its Prime Directive) and runs a **competition** across the whole swarm — every node writes its own answer, the best is adopted (the existing `deliberate` fan-out, now self-triggered). Logged as a `cross-query` event. This one costs tokens, so it lives below the budget gate; the *free* peer search round above does the same job for $0.
- **Cost-safe by design:** the LLM parts sit behind `HasBudgetAsync()`. Self-scaling and the free search rounds sit *above* it — spawning a node and searching a tensor cost nothing, so the mesh keeps forming and working after the day's cap is spent.
- Tuning: `HAL_TARGET_NODES` (mesh size), `HAL_CROSSQUERY_SECS` (LLM cross-query cadence), `HAL_PEERROUND_SECS` / `HAL_PEERSEARCH_SECS` (free peer rounds).
...
- **Fulfilment:** Stripe calls `/api/stripe-webhook`; the server verifies the `Stripe-Signature` (HMAC-SHA256 over `t.payload`, constant-time, 10-min replay window) with `STRIPE_WEBHOOK_SECRET`, then credits the wallet via `WalletCreditAsync`. This is the **only** path that can credit a purchased wallet — the browser can't.
...
Turning the dashboard from a passive readout into something you can *watch HAL think* on — and closing the one loophole where visitors could direct it for free forever.
- **The CRT now narrates the work.** The green phosphor terminal streams a live transcript read from the hive: matrix-race rounds (`matmul-round`), records, size-ladder convergence, novelty hits, accepted contributions, and steer builds — each timestamped with a status glyph — then prints the *actual latest artifact source* (a champion kernel **or** a freshly-invented tool). Because the dashboard and swarm are separate processes, every line is sourced from the shared Turso hive (new `matmul-round` / `steer-building` events; `showcase` table; `GetLatestArtifactAsync` picks the newer of tool-vs-kernel by ISO timestamp).
- **Click a build button, watch the code appear.** When a visitor commissions a tool, its generated C# is pushed to the `showcase` table (`PushShowcaseAsync` in `CommissionProposalAsync`) so the CRT shows it being written — the invention is visible, not hidden.
...
- **Real token enforcement.** Paid choices (invent-a-tool topics, boost) now cost a token from a **server-side, cookie-keyed wallet** (`wallet` hive table; `WalletBalanceAsync` / `WalletSpendAsync` / `WalletCreditAsync`). First visit grants `HAL_FREE_TOKENS` (default 3). Spending is enforced server-side: empty wallet → HTTP **402** and the action is refused; a queue-full failure **refunds** the token. The UI mirrors balance in a `⬡ tokens` pill, dims/padlocks choices you can't afford, and pops a **refuel CTA** instead of burning the click. Donations credit the wallet (`tokens` action on `/api/donate`), and `/api/wallet` returns the opaque `vid` so a Stripe checkout can reference it (cookie stays `HttpOnly`). The daily budget cap remains the hard backstop — tokens are just fair-use throttling, worth pennies.
...
### Going live — hal9001.io over HTTPS (bite 22)
...
- **The one gotcha worth remembering:** `HttpListener` matches its `http://localhost:8765/` prefix by **Host header**, so Caddy must rewrite it — `reverse_proxy localhost:8765 { header_up Host {upstream_hostport} }` — or every request 404s with "Not Found (Not Found)" even though TLS is fine.
- **Deploy loop:** edit locally → `dotnet publish -c Release -r linux-x64 --self-contained -o ./publish-linux` → `scp publish-linux/HAL9001.dll` to `/opt/hal9001/` → `chown hal:hal` + `systemctl restart`. Only `HAL9001.dll` changes (HTML/logic is baked into the managed assembly), and Linux lets you overwrite the running DLL; it takes effect on restart. A `deploy/` kit holds the unit files and `hal.env.example`.
